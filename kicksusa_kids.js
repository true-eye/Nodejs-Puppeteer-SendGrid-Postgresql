...
kicksusa_kids = async () => {
  ...
  while (page_index <= 20) {
    await page.goto(
      `https://www.kicksusa.com/sale-kids-grade-school.html?brands=78_63&footware_size=340_294_292_298_296_302_300_305&limit=72%3Fp%3D1&p=${page_index}`,
    ...
    const pageInfo = await page.evaluate(() => {
      ...
      const productDetails = document.querySelectorAll(
        '.products-grid > li > .item > .item-info',
      )
      for (var product of productDetails) {
        ...
        if (div_show && div_price) {
          ...
          const div_special_price = div_price.getElementsByClassName(
            'special-price',
          )[0]
          const div_regular_price = div_price.getElementsByClassName(
            'regular-price',
          )[0]

...
footpatrol = async () => {
  ...
  while (page_index <= 20) {
    await page.goto(
      `https://www.footpatrol.com/footwear/mens-footwear/brand/nike,jordan,new-balance/sale/?max=204`,
    )
    ...
    const pageInfo = await page.evaluate(() => {
      ...
      const productDetails = document.querySelectorAll(
        '.productListItem  > .itemContainer > .itemInformation',
      )
      for (var product of productDetails) {
        ...
        if (div_name && div_price) {
          ...
          if (
            div_sale_badge &&
            div_sale_badge.lastElementChild &&
            div_sale_badge.lastElementChild.lastElementChild
          ) {
            const productPrice =
              div_sale_badge.lastElementChild.lastElementChild.innerText
            products.push({
              ref: productRef,
              title: productTitle,
              price: productPrice,

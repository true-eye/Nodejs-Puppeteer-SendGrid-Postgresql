...
scrap_jimmyjazz_grade = async (func_name) => {
    console.log(func_name, '   Start   ');
    let siteURL = "http://www.jimmyjazz_grade.com/clearance?category=footwear"
    let ret = await manageDBFile.load_from_file("jimmyjazz_grade.json").then(prevList => {
        return jimmyjazz_grade().then((currentList) => {

            console.log(func_name, ' getCurrentProductList success : ', currentList.length);

            var changedFlag = false;
            let message = `<h2 style="background: white; color: red; text-align: center;"><a>www.jimmyjazz.com</a>   Kids</h2>`
            ...
            {
                manageDBFile.save_to_file("jimmyjazz_grade.json", currentList)
                    .then(res => {
                        console.log(res)
                    }).catch(err => {
            ...
            console.log(func_name, ' jimmyjazz_grade return error : ', err)
            return null;
        });
    }).catch(err => {
...
jimmyjazz_grade = async () => {
    ...
    while (1) {
        await page.goto(`https://www.jimmyjazz.com/clearance?category=footwear&gender=womens&page=${page_index}`, { waitUntil: 'domcontentloaded', timeout: 0 });
                ...
                if (product.children[2]) {
                    ...
                    if (div_productgrid_info.firstElementChild && div_productgrid_info.lastElementChild) {
                        ...
                        if (div_productgrid_brand_a && div_productgrid_title_a) {
                            productRef = "www.jimmyjazz_grade.com" + div_productgrid_brand_a.getAttribute('href');
                            ...
                            console.log('jimmyjazz_grade error occured line: 102')
                        }
                    ...
                    if (div_pricebox) {
                        if (div_pricebox.childElementCount == 2) {
                            ...
                            console.log('jimmyjazz_grade error occured line: 124')
                        }
                    }
                    ...
                    console.log('jimmyjazz_grade error occured')
                }
            }
...
exports.scrap_jimmyjazz_grade = scrap_jimmyjazz_grade;
exports.jimmyjazz_grade = jimmyjazz_grade;

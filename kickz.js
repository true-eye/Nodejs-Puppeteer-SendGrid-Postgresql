...
kickz = async () => {
  ...
  while (page_index <= 20) {
    await page.goto(
      `https://www.kickz.com/us/sale/jordan,nike/Men/shoes/c?selectedPage=${page_index}`,
    ...
    const pageInfo = await page.evaluate(() => {
      ...
      const productDetails = document.querySelectorAll(
        '#product_list_container > .product-info > .no-h-over',
      )
      ...
      return {
        products,
        bLastPage:
          btnNextPage[0].hasAttribute('href') == undefined ||
          btnNextPage[0].hasAttribute('href') == '',
      }
    })

...
scrap_jimmyjazz_women = async (func_name) => {
    console.log(func_name, '   Start   ');
    let siteURL = "http://www.jimmyjazz_women.com/clearance?category=footwear"
    let ret = await manageDBFile.load_from_file("jimmyjazz_women.json").then(prevList => {
        return jimmyjazz_women().then((currentList) => {
            ...
            {
                manageDBFile.save_to_file("jimmyjazz_women.json", currentList)
                    .then(res => {
                        console.log(res)
                    }).catch(err => {
            ...
            console.log(func_name, ' jimmyjazz_women return error : ', err)
            return null;
        });
    }).catch(err => {
                ...
                if (product.children[2]) {
                    ...
                    if (div_productgrid_info.firstElementChild && div_productgrid_info.lastElementChild) {
                        ...
                        if (div_productgrid_brand_a && div_productgrid_title_a) {
                            ...
                            console.log('jimmyjazz_women error occured line: 102')
                        }
                    ...
                    if (div_pricebox) {
                        if (div_pricebox.childElementCount == 2) {
                            ...
                            console.log('jimmyjazz_women error occured line: 124')
                        }
                    }
                    ...
                    console.log('jimmyjazz_women error occured')
                }
            }
...
exports.scrap_jimmyjazz_women = scrap_jimmyjazz_women;
exports.jimmyjazz_women = jimmyjazz_women;

...
undefeat_nike = async () => {
    ...
    while (1) {
        await page.goto(`https://undefeated.com/collections/sale/mens-footwear+nike?page=${page_index}`);

        const pageInfo = await page.evaluate(() => {
            let products = [];
            const productDetails = document.querySelectorAll('.product-grid-item > .product-content > .quick-shop-button > .pc-inner');
            ...
            for (var product of productDetails) {
                ...
                if (div_name && div_price) {
                    const productRef = "https://undefeat.com" + div_name.firstElementChild.getAttribute('href');

...
scrap_bdgastore_nike = async (func_name) => {
    console.log(func_name, '   Start   ');
    let message = `<h2 style="background: white; color: red; text-align: center;">bdgastore_nike.com Nike</h2>`
    let ret = await manageDBFile.load_from_file("bdgastore_nike.json").then(prevList => {
        return bdgastore_nike().then((currentList) => {
            ...
            if (true) {
                manageDBFile.save_to_file("bdgastore_nike.json", currentList)
                    .then(res => {
                        console.log(res)
                    }).catch(err => {
            ...
            console.log(func_name, ' bdgastore_nike return error : ', err)
            return null;
        });
    }).catch(err => {
...
bdgastore_nike = async () => {
    ...
    while (1) {
        await page.goto(`https://bdgastore.com/collections/sale#?Collections=Sale&Producttype=Shoes&Vendor=Nike&res_per_page=60&search_return=all&page=${page_index}`);
...
exports.scrap_bdgastore_nike = scrap_bdgastore_nike;
exports.bdgastore_nike = bdgastore_nike;